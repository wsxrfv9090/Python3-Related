...
def write_result(filename, ping):
	with open(filename, "W") as f:
		...
		for result in ping:
			f.write(result)
		f.write(f"End time {datetime.dateime.now()}")
...
def parse_arguments():
	parser = argparse.ArgumentParser(usage = '%(prog)s [options] <subnet>', description = 'ip checher', epilog = "python ipscanner.py 192.168.1 -f somefile.txt")
	...
	if not re.match(r"(\d{1,3}\.\d{1,3}\.\d{1,3})", args.subnet) \
	   or any(a not in range(1,255) for a in map(int, args.subnet.split(","))):
		parser.error("This is not a balid subnet")
